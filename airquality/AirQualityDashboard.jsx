...
import { AlertCircle, CheckCircle, Wind, Activity, Trash2, Power, Link as LinkIcon, RefreshCw, Download } from 'lucide-react';
...
// API Service (WITHOUT downloadCSV - we handle it directly in the component)
const api = {
...
const Alert = ({ type = 'info', children }) => {
  ...
  return (
    <div className={`px-4 py-3 rounded-lg border ${styles[type]} mb-4 flex items-start gap-2`}>
      {type === 'error' && <AlertCircle className="w-5 h-5 mt-0.5 flex-shrink-0" />}
      {type === 'success' && <CheckCircle className="w-5 h-5 mt-0.5 flex-shrink-0" />}
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
      <div className="bg-gradient-to-br from-blue-500 to-cyan-500 px-6 py-4 flex items-center justify-between">
        ...
        <div className={`px-3 py-1 rounded-full text-xs font-medium ${device.enabled ? 'bg-green-400 text-green-900' : 'bg-gray-400 text-gray-900'}`}>
...
const MappingCard = ({ mapping, onToggle, onDelete }) => {
  ...
  const handleDelete = async () => {
    if (!window.confirm('Are you sure you want to delete this mapping? The sensor will no longer control the plug.')) {
      return;
    }
    setIsDeleting(true);
    try {
...
export default function AirQualityDashboard() {
  ...
  // Load devices on mount and when view changes
  useEffect(() => {
    if (currentView === 'devices') {
      loadDevices();
      loadMappings();
    } else if (currentView === 'mappings') {
      loadMappings();
    }
  }, [currentView]);
  ...
  const loadDevices = async () => {
    ...
    setError(null);
    try {
      const data = await api.listDevices(userId);
  ...
  const loadMappings = async () => {
    ...
    setError(null);
    try {
      const data = await api.listMappings(userId);
  ...
  const loadTuyaDevices = async () => {
    ...
    setError(null);
    try {
      const data = await api.listTuyaDevices();
      setTuyaDevices(data.devices || []);
    } catch (err) {
      setError(`Failed to load Tuya devices: ${err.message}`);
      setTuyaDevices([]);
    } finally {
  ...
  const handleToggleMapping = async (mapping) => {
    try {
      await api.toggleMapping(userId, mapping.sensor_mac, mapping.tuya_device_id, !mapping.enabled);
      setSuccess(`Mapping ${!mapping.enabled ? 'enabled' : 'disabled'} successfully!`);
      loadMappings();
    } catch (err) {
      setError(`Failed to toggle mapping: ${err.message}`);
    }
  };

  const handleDeleteMapping = async (sensorMac) => {
    try {
      await api.deleteMapping(sensorMac);
  ...
  const openMapForm = (device) => {
    setMapForm({ sensorMac: device.sensor_mac, tuyaDeviceId: '', enabled: true });
    setShowMapForm(true);
    setError(null);
    setSuccess(null);
    loadTuyaDevices();
  ...
  const openCsvForm = (device) => {
    const { start, end } = getDefaultDateRange();
    setCsvForm({
      sensorMac: device.sensor_mac,
      sensorName: device.device_name || device.product?.en_name || 'Air Monitor',
      startDate: start,
      endDate: end
    });
    setShowCsvForm(true);
  ...
  const handleDownloadCSV = async (e) => {
    ...
    // Validate dates
    if (!csvForm.startDate || !csvForm.endDate) {
    ...
    try {
      // Direct fetch with full error handling
  ...
  const hasMappingForDevice = (sensorMac) => {
    return mappings.some(m => m.sensor_mac === sensorMac);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <span className="text-sm font-mono text-blue-900">Account: qingping_shared</span>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="space-y-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <p className="text-gray-600">Make sure sensors are already registered to the qingping_shared account.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {devices.map(device => (
                  <SensorCard
                    key={device.sensor_mac}
                    device={device}
                    onMapToPug={openMapForm}
                    onDownloadCSV={openCsvForm}
                    hasMappingExisting={hasMappingForDevice(device.sensor_mac)}
                  />
                ))}
              </div>
            )}
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleCreateMapping} className="p-8 space-y-6">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      {tuyaLoading ? "Loading Tuya devices..." : "Select a Tuya plug"}
                    </option>
                    {tuyaDevices.map(d => (
                ...
                <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  />
                  <label htmlFor="enabledCheckbox" className="text-sm font-medium text-gray-900">
                    Enable automation immediately (plug will turn on when PM2.5 ≥ 9)
                  </label>
                </div>
              ...
              <form onSubmit={handleDownloadCSV} className="p-8 space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-gray-700">
                    <strong>Note:</strong> The CSV will include all recorded sensor variables (PM2.5, PM10, CO2, temperature, humidity, battery) for the selected date range.
                  </p>
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    onClick={() => {
                      setShowCsvForm(false);
                      setCsvForm({ sensorMac: '', sensorName: '', startDate: '', endDate: '' });
                    }}
                    className="flex-1 py-3 px-6 bg-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-300 transition-all duration-200"
                  >
                    Cancel
